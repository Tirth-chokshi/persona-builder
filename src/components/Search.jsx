                                ...
                                <ul>
                                    {results.suggestions.map((company, index) => (
                                        ...
                                        >
                                            ...
                                            {company.image && (
                                                <img src={company.image} alt={company.title} className="w-16 h-16 mt-2 rounded-full" />
                                            )}
                                        </li>
                                    ))}
                                </ul>
                    ...
                    {selectedCompany && (
                        <div>
                            ...
                            <div className="border p-4 rounded shadow-md">
                                ...
                                <p><strong>Industry:</strong> {selectedCompany.industries || 'N/A'}</p>
                                <p><strong>Company Size:</strong> {selectedCompany.employees_on_li || 'N/A'}</p>
                                <p><strong>Headquarters:</strong> {selectedCompany.headquarters?.fullAddress || 'N/A'}</p>
                                <p><strong>Founded:</strong> {selectedCompany.founded_in || 'N/A'}</p>
                                <p><strong>Specialties:</strong> {selectedCompany.specialties ? selectedCompany.specialties.join(', ') : 'N/A'}</p>
                                <p><strong>Followers:</strong> {selectedCompany.followers || 'N/A'}</p>
                                <h4 className="font-semibold mt-2">About:</h4>
                                <p>{selectedCompany.description || 'No description available.'}</p>
                                {selectedCompany.logo && (
                                    <img src={selectedCompany.logo} alt={`${selectedCompany.company_name} logo`} className="w-24 h-24 mt-4 rounded-full" />
                                )}
                            </div>
                        </div>
                    )}
